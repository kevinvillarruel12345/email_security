...
const senEmail = require("../utils/senEmail");
...
const create = catchError(async (req, res) => {
  ...
  await senEmail({
    to: email,
    subject: "is my live",
    html: `<h1> hi ${firstName} ${lastName} somos los verificadores :v<h1> 
...
const verifyCode = catchError(async (req, res) => {
  ...
  if (!codeFound)
    return res.status(401).json({ message: " one error in your code" });
...
const getLoaggedUser = catchError(async(req, res) =>{
    const user = req.user;
    return res.json(user);
})
